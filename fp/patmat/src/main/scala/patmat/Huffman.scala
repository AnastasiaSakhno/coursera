...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs
      ...
      .sortWith((leaf1, leaf2) => leaf1.weight < leaf2.weight)
  ...
  def until(condition: List[CodeTree] => Boolean, action: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if (!condition.apply(trees)) until(condition, action)(action.apply(trees))
    else trees
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    val singletonList = until(singleton, combine)(orderedLeafList)
    singletonList.head
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertTailRec(currentTree: CodeTree, bits: List[Bit]): CodeTable = {
      currentTree match {
        case l: Leaf => List((l.char, bits))
        case f: Fork => mergeCodeTables(
          convertTailRec(f.left, bits ::: List(0)),
          convertTailRec(f.right, bits ::: List(1))
        )
      }
    }

    convertTailRec(tree, List())
  }
